...
namespace PNGimage {
  ...
  HSLAPixel & PNG::_getPixelHelper(unsigned int x, unsigned int y) const {
    if (width_ == 0 || height_ == 0) {
      cerr << "ERROR: Call to PNGimage::PNG::getPixel() made on an image with no pixels." << endl;
      assert(width_ > 0);
      assert(height_ > 0);
    }

    if (x >= width_) {
      cerr << "WARNING: Call to PNGimage::PNG::getPixel(" << x << "," << y << ") tries to access x=" << x
    ...
    if (y >= height_) {
      cerr << "WARNING: Call to PNGimage::PNG::getPixel(" << x << "," << y << ") tries to access y=" << y
